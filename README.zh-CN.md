...
你有过这种经历吗——打开目标网站打包好的 `main.chunk.js`，`Ctrl+F` 搜 `/api`，结果看到一堆文档里没提过的接口。管理员路径。还能用的 debug 开关。运气不好还能顺手捡到一个 AWS key。

`js-pulse` 就是因为我自己干这事干烦了写的。

它不神奇。不会给你发 payload。就是一个累了、偏执的 bug bounty hunter 看 JS 的方式，然后把它看到的东西记下来。
...
- 除非你让它拉，否则一个 HTTP 请求都不发（只分析你喂给它的 JS）
- 不搞漏洞利用——它负责给你信号，你去验证
- 不会假装发现 = 漏洞。高分只代表「值得细看」，不代表「拿下」
...
- **你需要 fuzz 和搞漏洞利用。** 这工具只读。用 Burp / Caido / nuclei。
- **你需要登录态爬。** js-pulse 不带认证。需要登录就用 headless browser。
- **你需要绕 CSP 找反射型 XSS。** js-pulse 不执行任何东西。

它就是个 recon 工具。是 engagement 开头跑的，不是拿 bounty 的那个。
...
可能永远不做的事（违反「只读」原则）：动态 JS 执行、headless 渲染、需要浏览器的任何东西。
...
只能在你有授权的系统上跑。作者不对你用这工具干的事负责，「我只是读了一下 JavaScript」这种借口不灵。
